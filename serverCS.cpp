...
{
    int serverCSSock, serverCSLen;
    socklen_t fromLen;
    sockaddr_in serverCS;
    sockaddr_in fromM;
    char buff[1024];

    serverCSSock = socket(AF_INET, SOCK_DGRAM, 0); // creating a UDP socket
    if (serverCSSock < 0){
        error("Error opening sokcet");
    } 
    serverCSLen = sizeof(serverCS);
    memset(&serverCS, 0, serverCSLen);
    serverCS.sin_family=AF_INET;
    serverCS.sin_addr.s_addr=INADDR_ANY;
    serverCS.sin_port=htons(22887); // 22000 + 887 (last 3 digits of my ID)
    if (bind(serverCSSock, (sockaddr *)&serverCS, serverCSLen)<0) {
        error("Binding failed.");
    }
    cout << "The ServerCS is up and running using UDP on port 22887." << endl;
    processFile(); //read file cs.txt and store in a vector
    fromLen = sizeof(struct sockaddr_in);
    while(true){
        memset(buff,0,1024);
        int requestReceived = recvfrom(serverCSSock, buff, 1024, 0, (sockaddr *)&fromM, &fromLen);
        ...
        if(courseRequest[0] == '0'){
            ...
            string category = courseRequest.substr(courseRequest.find(",")+1, courseRequest.size());
            ...
            int sendCourseDetails = sendto(serverCSSock, toSend.c_str(), toSend.size()+1, 0, (sockaddr *)&fromM, fromLen);
            ...
            cout << "The ServerCS received a request from the Main Server about the multiple courses." << endl; //modified slightly for multiple courses
            vector<string> courseList = convertToList(courseRequest);
            string courseDetails = getMultipleCourseDetails(courseList);
            int sendCourseDetails = sendto(serverCSSock, courseDetails.c_str(), courseDetails.size()+1, 0, (sockaddr *)&fromM, fromLen);
            if (sendCourseDetails < 0) {
                error("Failed to send");
            }
            else{
...
//loads file cs.txt and stores each line in the vector courses
void processFile(){
    string checkLine;
    ifstream csFile ("cs.txt");
    if(csFile.is_open()){
        while(getline(csFile, checkLine)){
            courses.push_back(checkLine);
        }
        csFile.close();
    }
    else{
        error("Could not open file cs.txt");
    }

}

//check if courseCode matches with any of the courseCodes in the CS list in cs.txt, and return the details based on the category
string getCourseDetails(string courseCode, string category){
